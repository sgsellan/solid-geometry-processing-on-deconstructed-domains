%%%%%%%%%%%%%%-3D BILAPLACE EQUATION CONVERGENCE TEST-%%%%%%%%%%%%%%%%%%%%%
%   This script is designed to exhaustively test convergence of the 3D
%bipoisson equation with Dirichlet and Neumann boundary conditions, using
%the Hessian energy described in [Stein et al. 2017].
%                         lap lap u = f,
%               u|_{\partial\Omega} = g,
...
for s=1:4
    ...
    VV = {VA,VB};
    FF = {FA,FB};
    ZZd = overlap_bipoissonh3(VV,FF,bc_fun,rhs_fun,norm_der,'Method','dirichlet');
    ZZn = overlap_bipoissonh3(VV,FF,bc_fun,rhs_fun,norm_der,'Method','naive');
    ZZs = overlap_bipoissonh3(VV,FF,bc_fun,rhs_fun,norm_der,'Method','strict');
    ZZgt = overlap_bipoissonh3({V},{F},bc_fun,rhs_fun,norm_der);
